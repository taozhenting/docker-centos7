...
COPY ./php-7.1.13.tar.gz /usr/local/src/
...
RUN tar zxvf php-7.1.13.tar.gz
WORKDIR php-7.1.13
RUN ./configure \
--prefix=/opt/php7.1.13 \
--with-curl \
--with-freetype-dir \
--with-gd \
--with-gettext \
...
COPY ./php.ini /opt/php7.1.13/lib/php.ini
COPY ./php-fpm.conf /opt/php7.1.13/etc/php-fpm.conf
COPY ./www.conf /opt/php7.1.13/etc/php-fpm.d/
RUN mv /etc/nginx/nginx.conf /etc/nginx/nginx.conf.bak
COPY ./nginx.conf /etc/nginx/
COPY ./test.php /usr/share/nginx/html/
RUN cp /usr/local/src/php-7.1.13/sapi/fpm/init.d.php-fpm /etc/init.d/php-fpm
